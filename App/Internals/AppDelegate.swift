...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    var window: UIWindow?
    let crashedNotificationKey = "com.abstraqara.crashNotificationKey"
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        if errorLog.count < 1 {
            let errorItm = RLM_Errors()
            do {
                try realm.write {
                    self.realm.add(errorItm)
                }
            } catch {
                print("Error: \(error)")
            }
        ...
        crashedLastTime = errorLog.first!.crashed
